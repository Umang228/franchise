...
router.post('/add-product', (req, res) => {
    ...
    const sql =
      'INSERT INTO products (productName, facultyName, productID, productType, course, group_name, subject, deliveryType, isFranchise, isWhatsapp, priceUpdate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
  ...
  db.query(franchiseSql, franchiseValues, (err, franchiseResult) => {
    ...
    db.query(userSql, userValues, (userErr, userResult) => {
      if (userErr) {
        ...
        return res.status(500).json({ message: 'Error creating franchise user account' });
      }

      return res.status(200).json({ message: 'Franchise created successfully' });
    });
  });
});
